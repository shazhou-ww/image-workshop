...
import { controlSketch } from '@image-workshop/mcp-tools';
import {
  callStabilityApi,
...
/**
 * Request body for control/sketch
 */
export interface ControlSketchRequest {
  image: string;
  prompt: string;
 ...
 */
export const toolSchema = controlSketch;

/**
 * Parse and validate request body
 */
function parseRequest(body: string | null): ControlSketchRequest {
  if (!body) {
...
export const handler = async (
  ...
  try {
    ...
    console.log('Calling Stability AI control/sketch...');
    console.log(`Prompt: ${request.prompt.substring(0, 100)}...`);

    // Support both base64 and URL inputs
    const imageBlob = await createImageBlobAsync(request.image, 'sketch.png');
    ...
    console.log(`Sketch-to-image complete. Seed: ${response.seed}`);
