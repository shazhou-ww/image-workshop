...
import { controlStyle } from '@image-workshop/mcp-tools';
import {
  callStabilityApi,
...
/**
 * Request body for control/style
 */
export interface ControlStyleRequest {
  image: string;
  prompt: string;
 ...
 */
export const toolSchema = controlStyle;

/**
 * Parse and validate request body
 */
function parseRequest(body: string | null): ControlStyleRequest {
  if (!body) {
...
export const handler = async (
  ...
  try {
    ...
    console.log('Calling Stability AI control/style...');
    console.log(`Prompt: ${request.prompt.substring(0, 100)}...`);

    // Support both base64 and URL inputs
    const imageBlob = await createImageBlobAsync(request.image, 'style.png');
    ...
    console.log(`Style-guided generation complete. Seed: ${response.seed}`);
